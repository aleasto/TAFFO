...
Value *FloatToFixed::genConvertFloatToFix(Value *flt, const FixedPointType &fixpt, Instruction *ip)
{
  ...
  if (!destt->isFloatingPointTy()) {
    if (fixpt.isPosit()) {
      Value *positVar = PositBuilder(builder, fixpt).CreateConstructor(flt);
      return cpMetaData(positVar, flt, ip);
    }
...
Value *FloatToFixed::genConvertFixToFloat(Value *fix, const FixedPointType &fixpt, Type *destt, Instruction *ip)
{
  ...
  if (isa<Instruction>(fix) || isa<Argument>(fix)) {
    ...
    if (fixpt.isPosit()) {
      return PositBuilder(builder, fixpt).CreateConv(fix, destt);
    ...
    if (fixpt.isPosit()) {
      return PositConstant::FoldConv(getModule(), cst->getContext(), fixpt, cst, destt);
    } // otherwise fixpt.isFixedPoint()
